...
var template=function(){return {name:"",pet:"",mount:"",description:"",type:""}};
var P=0,S=false,output=[],CO=template,name=false,mount=false,pet=false;
for(var i=0;i<data.length;i++){
	if(data.substring(i,i+5)=="name="&&!S){ //attribute
		S=true;P=i+6;i+=5;name=true; //always first obj, clear last obj
	}else if(data.substring(i,i+4)=="pet="&&!S){ //attribute
		S=true;P=i+5;i+=4;pet=true;
	}else if(data.substring(i,i+6)=="mount="&&!S){ //attribute
		S=true;P=i+7;i+=6;mount=true;
	}else if(data[i]=='"'&&S){ //end attribute
		if(name)CO.name=data.substring(P,i);
		if(pet)CO.pet=data.substring(P,i);
		if(mount){
			CO.mount=data.substring(P,i);
			S=true;P=i+2; //Start type text after mount attribute
		}else{S=false;}//end if
		name=false,pet=false,mount=false; //reset for new stat
	}else if(data.substring(i,i+12)=="<description"&&!S){ //field
		S=true;P=i+13;i+=12;
	}else if(data.substring(i,i+13)=="</description"&&S){ //end field
		CO.description=data.substring(P,i);S=false;
	}else if(data.substring(i,i+6)=="</type"&&S){ //end type
		CO.type=data.substring(P,i);S=false;
	}else if(data.substring(i,i+6)=="</item"){ //end item
		output.push(CO);
...
for(entry in output){
	outputData+="{name:\""+output[entry].name+"\",<br/>"+
	            "&nbsp;type:{<br/>"+
			    "&nbsp;pet:"+output[entry].pet+",<br/>"+
			    "&nbsp;&nbsp;&nbsp;&nbsp;mount:"+output[entry].mount+",<br/>"+
			    "&nbsp;&nbsp;&nbsp;&nbsp;description:\""+output[entry].type+"\",<br/>"+
			    "&nbsp;},<br/>"+
			    "&nbsp;description:\""+output[entry].description+"\"<br/>"+
			    "}";
