...
export class Map {
  ...
  findPath ({ x1 = 0, y1 = 0, x2 = 0, y2 = 0, test = () => true, map = this } = {}) {
    ...
    const openList = new Heap([], (a, b) => a.f === b.f, (a, b) => b.path.f - a.path.f);
    const abs = Math.abs; // shorten reference
    const clone = this.clone(); // so we can mutate it and destroy it when done
    const SQRT2 = Math.SQRT2; // shorten reference
