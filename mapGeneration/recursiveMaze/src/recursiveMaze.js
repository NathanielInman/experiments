...
export function recursiveMaze (map) {
  ...
  const sx = Math.floor(Math.random() * map.width);
  const sy = Math.floor(Math.random() * map.height);

  let x = sx; let y = sy;
  let direction; // represents the random chosen direction
  let sector = map.getSector({ x, y }); // represents the sector we're testing
