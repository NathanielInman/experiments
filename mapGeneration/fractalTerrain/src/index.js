...
if (!easel.activated) {
  ...
  const map = new Map(settings.map.width, settings.map.height);

  bsp(map);
  applyGaussianBlur(map);
  normalize(map);
  easel.onDraw = () => {
    const ph = easel.viewport.h / map.height; const pw = easel.viewport.w / map.width;
    let c; let c2; // represents color
