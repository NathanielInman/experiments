...
function tick(c1, c2) {
  ...
  if (r(0, 2, 1) === 0) {
    ...
    if (c2.health > 0) setTimeout(function () {
      outputCache.push(c2.process(c1).split("|||").clean());
    }, 500);
  } else {
    //c2 -> c1
    outputCache.push(c2.process(c1).split("|||").clean());
    if (c1.health > 0) setTimeout(function () {
      outputCache.push(c1.process(c2).split("|||").clean());
    }, 500);
  } //end if
...
(function (Database) {
  var creatures = [{
    ...
    amplitude: { fire: 0, water: 0, air: 0, earth: 2, spirit: 0 },
    symbol: "R",
    description: "Barbegazis are a type of gnome that can be found at the tops of frozen " + "mountains. They are rarely seen, these creatures enjoy helping each " + "other and set subtle warnings of trespassers. It's common to see " + "Barbegazi form as groups against trespassers to more quickly dispatch " + "an enemy that they find formidable. Barbegazi's are the height of " + "quicklings and weigh no more than 30 stones."
  }, {
    name: "Barghest",
    weight: 80,
    height: 30,
    ...
    amplitude: { fire: 0, water: 0, air: 0, earth: 0, spirit: 0 },
    symbol: "m",
    description: "Chimaera's are part lion, goat, and snake. Chimaera's are the second " + "most popular beast to guard portals, next to dragons. They are all " + "females and mate solely with dragons. With the decline of dragons " + "came the decline of Chimaeras. A chimaera stands 80 stones tall and " + "weighs roughly 450 stones."
  }, {
    name: "Cinnamologus",
    weight: 180,
    height: 80,
    ...
    symbol: "W",
    description: "Wyverns are colossal, ancient, dragons who have aged so much they " + "lost theirs wings and tails have enlarged. They look more-or-less " + "like Worms, incredibly large ones at that. They sometimes burrow " + "within the ground as the Plains of Endless Despair can be quite " + "ridden with magical properties that are incredibly dangerous. They " + "protect the drakes that sour about the area of the Plains of Endless " + "Despair as if they were their own children. Wyverns can talk " + "telepathically."
  }, {
    name: "Xolotl",
    weight: 80,
    height: 30,
...
(function (Database) {
  ...
  var immolation = new SpellEffect(function () {
    ...
    if (this.target.effects["immolate " + this.element] && this.target.effects["immolate " + this.element].stacks < 5) {
      ...
      this.target.effects["immolate " + this.element] = {
        ...
        script: this.actor.effects["elemental precision"] ? function (name) {
      ...
      this.target.effects["immolate " + this.element] = {
        timer: 5,
        stacks: 1,
        script: this.actor.effects["elemental precision"] ? function () {
          var dmg = 10 + r(1, 4, 1);
          var element = this.spell.split(" ")[1];
          if (this.actor.effects["vicerating " + element]) {
            dmg += 10 * this.actor.effects["vicerating " + element].stacks;
  ...
  var viceration = new SpellEffect(function () {
    ...
    // if this affect already exists on the target, then ensure
    // that it stacks for a maximum of 5 times
    if (this.target.effects["vicerating " + this.element] && this.target.effects["vicerating " + this.element].stacks < 5) {
...
(function (Database) {
   ...
   */
  var Elementalist = (function (_Database$Class) {
    function Elementalist(actor, specialty) {
      var t;
      this.actor = actor;
    ...
    _inherits(Elementalist, _Database$Class);

    _prototypeProperties(Elementalist, null, {
      ai: {
        value: function ai(target) {
          var result = [];
          if (r(0, 2, 1) === 0) {
            ...
            if (!this.actor.effects["elemental precision"]) {
              ...
              if (this.specialty == "fire") {
                if (!target.effects["vicerating fire"]) {
                  //step 1
                ...
                if (!target.effects["vicerating water"]) {
                  //step 1
                ...
                if (!target.effects["vicerating air"]) {
                  //step 1
                ...
                if (!target.effects["vicerating earth"]) {
                  //step 1
                ...
                if (!target.effects["vicerating spirit"]) {
                  //step 1
                  result.push(this.actor.cast("vicerating darkness", target));
                } else if (!target.effects["immolate spirit"]) {
                  //step 2
...
(function (Database) {
  var ImageCollection = (function () {
    ...
    _prototypeProperties(ImageCollection, null, {
      get: {
        value: function get(name) {
          return this.images[name] || (function () {
            throw "Image requested doesn't exist.";
          })();
        },
        writable: true,
        enumerable: true,
      ...
      draw: {
        value: function draw(name, snipx, snipy, snipw, sniph, x, y, w, h) {
...
(function (Database) {
  var Spell = (function () {
    ...
    _prototypeProperties(Spell, null, {
      cast: {
