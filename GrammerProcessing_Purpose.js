...
var core={
	...
	matches:function(dataSet,testString){
		for(var i=0;i<dataSet.length;i++){
			if(testString.toLowerCase()==dataSet[i])return true;
		} //end for
		return false;
	},
	library:{
		...
		conjunction:{
			...
			adverb:['accordingly','finally','hence','hence','instead','now','nevertheless','until','also','however','likewise','next','still',
					...
					'furthermore','namely','otherwise','thus','why']
		},
		verb:[  'could','would','should','can',"can't",
				'abide','abided','abode','abidden','abides','abiding',
				...
				'discredit','discredits','discredited','discrediting',
				...
				'stretch','stretched','streches','stretching',
	...
	process:function(message){
		...
		(function(msg){
			...
			for(var i=0;i<words.length;i++){
				...
				if(words[i].type==' '&&words[i].word[0].toUpperCase()==words[i].word[0])words[i].type+=gc('#FF0','(Noun)');
			} //end for
			for(var i=0;i<words.length;i++){
				/* WHATEVER IS LEFT MUST BE EITHER ADJECTIVE OR NOUN */
				if(words[i].type==" "&&i!=words.length-1&&words[i+1].type.indexOf("(Conjunction")>=0){
					...
					words[i].type=gc('#4AF','(Adjective)');
				}else if(words[i].type==" "&&i!==words.length-1&&words[i+1].type.indexOf("(Verb)")>=0){
			...
			(function(){
				var noun="",associations=[];
				if(words[words.length-1].word.indexOf(".")>=0){
					core.pl("[STATEMENT]");
					words[words.length-1].word=words[words.length-1].word.replace(".","");
				}else if(words[words.length-1].word.indexOf("?")>=0){
					core.pl("[QUESTION]");
					words[words.length-1].word=words[words.length-1].word.replace("?","");
				} //end if
				for(var i=0;i<words.length;i++){
					if(!noun&&words[i].type.indexOf("(Noun)")>=0){
						noun=words[i].word;
					}else if(noun&&words[i].type.indexOf("(Verb)")>=0){
						if(associations.length>0){
							core.pl(noun+":"+associations.join());
							associations=[];
						} //end if
						core.pl(noun+"->"+words[i].word);noun="";
					}else if(words[i].type.indexOf("(Adjective)")>=0){
						associations.push(words[i].word);
					} //end if
					if(noun&&i==words.length-1){
						for(var j=words.length-1;j>=0;j--){
							if(words[j].type.indexOf("(Verb)")>=0){
								core.pl(noun+":"+associations.join());
								core.pl(words[j].word+"->"+noun);
								break;
							} //end for
						} //end for
					}//end if
...
(function(){
	...
	setTimeout(function(){core.process('A large and gray cat named Sampson stretched out on the cool ground.');},10);
})();