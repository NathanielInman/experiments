/**********************************************************************************\
 ...
 \*********************************************************************************/
export var Ion=function(q,s,x,y,dx,dy){
  this.particle=[];
  this.clear=true;
  this.quantity=q||1;
  this.sx=x||0;
...
Ion.prototype.getNew=function(atom){
  ...
  return {
    ...
    x:sx,
    y:sy,
    dx:typeof this.dx            =='function'?this.dx(sx):this.dx,
    dy:typeof this.dy            =='function'?this.dy(sy):this.dy,
...
Ion.prototype.draw=function(atom){
  if(this.particle[atom].image){
    this.easel.ctx.drawImage(this.particle[atom].image,this.particle[atom].x-this.particle[atom].imageWidth/2,this.particle[atom].y-this.particle[atom].imageHeight/2);
  }else{
    this.easel.ctx.fillRect(this.particle[atom].x,this.particle[atom].y,this.particle[atom].s,this.particle[atom].s);
  } //end if
};
...
Ion.prototype.process=function(){
  var that=this;
  if(this.clear){
    this.easel.ctx.fillStyle=this.clearColor;
...
Ion.prototype.getFrame=function(){
  ...
  for(let atom in this.particle){
    ...
    this.easel.ctx.fillStyle=this.color(p);
    if(p.c==p.d)continue; //skip a done movement process
    this.wind(atom);
    this.draw(atom);
    this.modulate(atom);
    if(p.x<0||p.y<0||p.x>this.easel.viewport.w||p.y>this.easel.viewport.h)this.onEscape(atom);
