...
var Point=function(x, y){
	this.x = x;
	this.y = y;
	this.dx = Math.random()*2|0;
	this.dy = Math.random()*2|0;
};
Point.prototype.distance = function(a, b){
   return(Math.sqrt( (b.x-a.x)*(b.x-a.x) + (b.y-a.y)*(b.y-a.y) ));
};
Point.prototype.move=function(){
	if(this.dx==0){this.x--;}else{this.x++;}
	if(this.dy==0){this.y--;}else{this.y++;}
	if(this.x<10||v.w-this.x<10)this.dx^=1;
	if(this.y<10||v.h-this.y<10)this.dy^=1;
};
...
var w=window.innerWidth;
	...
	for(i=0; i<edges.length; i++){
		var e = edges[i];
		c.beginPath();
		/*if(e.start.y>w/4&&e.start.y<w/4*2){
			c.strokeStyle="#00F";
		}else{
			c.strokeStyle="#0F0";
		} //end if */
